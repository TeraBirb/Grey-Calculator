...
function assignNum(numVar, num) {
  numVar.addEventListener("click", () => {
    if (
      (w < 500 && curInput.innerHTML.length < 10) ||
      (w >= 500 && curInput.innerHTML.length < 13) ||
      ...
      alert(
        "Oops, the characters don't fit! Increase window size and refresh or use a big boy calculator."
      );
    }
  });
}
